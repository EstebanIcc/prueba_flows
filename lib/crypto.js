...
const decryptAESKey = (encryptedAESKey) => {
    try {
        ...
        for (const format of formats) {
            try {
                console.log('Intentando formato:', format.type);
                decryptedPrivateKey = crypto.createPrivateKey({
                    ...format,
                    passphrase: passphrase
                });
                console.log('Formato exitoso:', format.type);
                break;
            } catch (error) {
                console.log('Error con formato', format.type, ':', error.message);
                lastError = error;
            }
        }
        ...
        console.log('Longitud del buffer encriptado:', buffer.length);

        // Usar directamente el método 4 (RSA_PKCS1_OAEP_PADDING)
        console.log('Usando método 4 (RSA_PKCS1_OAEP_PADDING) directamente');
        ...
        console.log('Clave AES desencriptada exitosamente con método 4');
        return decryptedKey;
    } catch (error) {
 ...
 */
const encryptResponse = (payload, aesKey, iv) => {
    try {
